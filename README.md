...
- GitHub Actions 每 15 分钟触发一次；窗口外按预约时间快速退出，不占用 runner。
- 剩余时间进入续期窗口前 2 小时开始检查；任何一次检查发现剩余时间低于 4 小时，立即续期。
- 单个任务超时限制为 10 分钟；续签失败会把 Actions 标记为失败，等待下一轮 15 分钟 cron 自动重试。
